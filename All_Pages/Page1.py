...
def app():
    ...
    # with st.sidebar:
    selected1 = option_menu(None, ["Sales Cumulative Database", 'Product Data','Month-Wise Data','City-Wise Data'], menu_icon="cast", default_index=0,orientation="horizontal")
    ...
    if selected1 == "Sales Cumulative Database":
        st.header("Sales Annual Dataset [2019]")
        st.dataframe(df1,use_container_width=False)
        
    if selected1 == "Product Data":
        st.header("Product Analysis")
        st.dataframe(product_counts_df, use_container_width=True)
    
    if selected1 == "Month-Wise Data":
        st.header("Month Wise Analysis")
        st.dataframe(df2, use_container_width=True)
    
    if selected1 == "City-Wise Data":
